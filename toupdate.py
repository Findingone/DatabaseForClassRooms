...
def addProfTable():
    ...
    # handle sqlite
    database = sqlite3.connect(os.path.join(
        'database', instituteCode, Database))
...
def updateProfTable():
    ...
    for prof in data:
        sql_command = """INSERT OR IGNORE INTO {}teachingFaculty VALUES ('{}' , '{}' , '{}', 'NA' )""".format(
            instituteCode, prof[0], prof[1], prof[2])
        crsr.execute(sql_command)
        sql_command = "CREATE TABLE IF NOT EXISTS {}teacher{}(courseName VARCHAR unique)".format(
            instituteCode, prof[0])
        crsr.execute(sql_command)
        for j in range(len(prof)-3):
            sql_command = "INSERT or IGNORE INTO {}teacher{} VALUES('{}')".format(
...
def getStudentAttendance():
    ...
    for course in courses:
        if course[1] == 0:
            ...
            sql_command = "select * from {}{} where RollNo = '{}'".format(instituteCode,
                                                                          course[0], rollNo)
            crsr.execute(sql_command)
            ...
            if(len(attendanceRecord) > 3):
                data.append(course[0] + ": " + str(attendance /
                                                   (len(attendanceRecord)-3)*100) + "%")
    database.close()
...
def profAddCourse():
    ...
    database = sqlite3.connect(os.path.join(
        'database', instituteCode, Database))
    crsr = database.cursor()
    sql_command = "INSERT INTO {}teacher{} VALUES('{}')".format(instituteCode,
                                                                profId, courseName)
    crsr.execute(sql_command)
...
def profRemoveCourse():
    ...
    database = sqlite3.connect(os.path.join(
        'database', instituteCode, Database))
    crsr = database.cursor()
    sql_command = "DELETE from {}teacher{} where courseName = '{}'".format(instituteCode,
                                                                           profId, courseName)
    crsr.execute(sql_command)
...
def updateSemester():
    ...
    for i in rollNumbers:
        rollNumber = i[0]
        sql_command = "select course from {}{} where active = 1".format(instituteCode,
                                                                        rollNumber)
        crsr.execute(sql_command)
        courses = crsr.fetchall()
        for course in courses:
            ...
            if(len(details[0]) != 3):
                ...
                sql_command = "update {}{} set Active = 0 , Attendance = {}". format(instituteCode,
    ...
    for professor in professors:
        sql_command = "DROP TABLE {}teacher{}".format(
            instituteCode, professor[0])
        crsr.execute(sql_command)
